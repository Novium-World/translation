...
const StartupContainer = () => {
    ...
    return !data ? (
        ...
        <ServerContentBlock title={'Startup Settings'} showFlashKey={'startup:image'}>
            <div css={tw`md:flex`}>
                <TitledGreyBox title={'Startup Befehl'} css={tw`flex-1`}>
                    <div css={tw`px-1 py-2`}>
                        <p className='font-mono bg-neutral-900 rounded py-2 px-4' css='background-color:var(--input)'>{data.invocation}</p>
                    </div>
                </TitledGreyBox>
                <TitledGreyBox title={'Docker Image'} css={tw`flex-1 lg:flex-none lg:w-1/3 mt-8 md:mt-0 md:ml-10`}>
                    {Object.keys(data.dockerImages).length > 1 && !isCustomImage ? (
                        <>
                            ...
                            Dies ist eine erweiterte Funktion, mit der man ein Docker-Image auswählen kann, das beim Ausführen dieser Serverinstanz verwendet werden soll.
                            </p>
                        </>
                    ) : (
                        <>
                            ...
                            {isCustomImage && (
                                <p css={tw`text-xs text-neutral-300 mt-2`}>
                                    Dieses {"server's"} Docker image wurde durch einen Admin manuell geändert.
                                </p>
                            )}
                        </>
                    )}
            ...
            <h3 css={tw`mt-8 mb-2 text-2xl`}>weitere optionen</h3>
