...
export default () => {
                ...
                if (name.match('.*-(P[0-9]*|S[0-9]*|M[a-zA-Z0-9]*)')) {
                    ...
                    let req = await fetch(
                        `https://spigot.fyrehost.net/v2/search/resources/${
                            plugins.data.data[i].attributes.name.split('.')[0]
                        }?size=1`
                    );
    ...
    async function submitPolymart() {
        ...
        const polymartLoading = setInterval(async () => {
            if (await verifyToken(res.token)) {
                setLoading(false);
                setPolymartAuth(true);
                saveToken(res.token);
                clearInterval(polymartLoading);
            }
        }, 1000);
    }
        ...
        <>
            ...
            <div css={tw`mx-auto my-5`}>
                ...
                {!plugins || plugins.length === 0 ? (
                    ...
                    <>
                        <div css={tw`grid lg:grid-rows-4 lg:grid-cols-4 mt-4 mx-10 gap-4`}>
                            {plugins.map((plug) => (
                                <PluginContainer
                                    plugin={plug}
                                    version={version}
